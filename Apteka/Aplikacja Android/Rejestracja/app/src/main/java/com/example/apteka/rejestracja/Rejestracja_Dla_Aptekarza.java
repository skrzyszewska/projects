                    ...
                    if (Haslo1.equals(PowtorzHaslo) && PowtorzHaslo.equals(Haslo1)){
                        if (Jest3 == true){
                            if (CheckEditTextEmpty == true && Jest == true && Jest2 == true){
                                ...
                                String query = "INSERT INTO" + " Aptekarze " + "(Login, Haslo, Email, Podpis, Imie, Nazwisko, Telefon, Miejscowosc, KodPocztowy, Adres, DataUrodzenia,Aktywny,KodAktywacyjny) "
                                        + "Values ('" + Login1 + "', '" + nazwa + "', '" + Email1 + "', '" + Podpis1 + "','" + Imie1 + "','" + Nazwisko1 + "','" + Telefon1 + "','" + Miejscowosc1 + "','" + KodPocztowy1 + "','" + Adres1 + "','" + DataUrodzenia1 + "','" + 0 + "','" + kodaktywacyjny + "')";
                                ...
                                int idik = zwroc_ostatni_aptekarz_id();

                                Toast.makeText(Rejestracja_Dla_Aptekarza.this, "pokaz id" + idik , Toast.LENGTH_LONG).show();
                        ...
                        else{ Toast.makeText(Rejestracja_Dla_Aptekarza.this, "Zły Podpis!", Toast.LENGTH_LONG).show();}
                    }
                    else{ Toast.makeText(Rejestracja_Dla_Aptekarza.this, "Hasła nie są równe! ", Toast.LENGTH_LONG).show();}
                }
                else{ Toast.makeText(Rejestracja_Dla_Aptekarza.this, "Błąd w formularzu! Login ma mieć długość min. 4 znaki, a Hasło 8! ", Toast.LENGTH_LONG).show();}
            }
            else{ Toast.makeText(Rejestracja_Dla_Aptekarza.this, "Zły podpis! ", Toast.LENGTH_LONG).show();}
            }
        });
    ...
    */
    ...
    public void sprawdzPodpis(String str){
        ...
        if ((str.length() == 6) && (str.charAt(2) == 'p' || str.charAt(2) == 'i'  ||  str.charAt(2) == 'k'
                ...
                (str.charAt(4) >= '0' && str.charAt(4) <= '9') && (str.charAt(5) >= '0' && str.charAt(5) <= '9') && (x == 15)){
            Jest3 = true;
        }else
            Jest3 = false;
    }

    int zwroc_ostatni_aptekarz_id(){
        JSONArray tab2 = new db_connect().getResults("SELECT * FROM Aptekarze");
        try{
