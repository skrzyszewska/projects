...
function my_wp_nav_menu_args( $args = '' ) { 
	...
	global $wpdb;
if( $zalogowany == 1 ) {
	$user = $wpdb->get_row( "SELECT * FROM Aptekarze WHERE ID = $ID" );
	echo "<center>Witaj $user->Imie</center><p>";
    $args['menu'] = 'Zalogowany-aptekarz';
} elseif( $zalogowany == 2 ) { 
	$user = $wpdb->get_row( "SELECT * FROM Klienci WHERE ID = $ID" );
	echo "<center>Witaj $user->Imie</center><p>";
    $args['menu'] = 'Zalogowany-klient';
} else {
	$args['menu'] = 'Gosc';
}
    return $args;
