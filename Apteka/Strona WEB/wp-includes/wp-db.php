...
class wpdb {
	...
	public function db_log($query) {
		...
		if ($_SESSION['Zalogowany'] == 1 )
			$user = "Aptekarz_".$_SESSION['ID'];
		elseif ($_SESSION['Zalogowany'] == 2 )
			$user = "Klient_".$_SESSION['ID'];
		else
			$user = "Gość";
		
		if (strpos($query, 'wp_') === false && strpos($query, 'SELECT') === false && strpos($query, 'select') === false) {
			$data_teraz = date("Y-m-d H:i:s");
			$log_query = "INSERT INTO `Logi` (`Query`,`Platforma`, `Uzytkownik`, `IP`, `Data`) values (\"$query\", 'Web', '$user', '$ip', '$data_teraz')";	
			$this->_do_query( $log_query );
		}
	}
